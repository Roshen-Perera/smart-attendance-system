...
export const StudentFacePage: React.FC = () => {
  ...
  const handleGenerateEmbedding = async () => {
    ...
    try {
      ...
      toast.success('AI face vector embedding generated successfully!');
      loadStudentData();
    } catch (err: any) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Must be a clear front-facing portrait photo (JPG/PNG)</p>
            </div>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5 flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">128-dimensional deep learning facial vector</p>
              </div>
            </div>

            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400">Face Image Uploaded:</span>
                <span className={images.length > 0 ? 'text-emerald-400 font-semibold flex items-center gap-1' : 'text-amber-400 font-semibold'}>
                  {images.length > 0 ? (
                    <>
                      <CheckCircle2 className="w-3.5 h-3.5" /> Yes ({images.length})
                    </>
                  ) : (
                    'No Photo'
                  )}
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className={embeddings.length > 0 ? 'text-emerald-400 font-semibold flex items-center gap-1' : 'text-rose-400 font-semibold'}>
                  {embeddings.length > 0 ? (
                    <>
                      <ShieldCheck className="w-3.5 h-3.5" /> Ready for AI
                    </>
                  ) : (
                    'Pending Embed'
                  )}
          ...
          <div className="space-y-3">
            ...
            >
              {isGenerating ? 'Extracting 128-d Vector...' : 'Generate AI Face Embedding'}
            </button>
